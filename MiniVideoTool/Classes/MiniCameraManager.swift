...
class MiniCameraManager: NSObject {
    ...
    let orientation = MiniDeviceOrientation()
    ...
    func setupView() {
        focusImageView.image = UIImage(named: "sight_video_focus")
    ...
    func pickImage(complete: @escaping (String) -> ()) {
        ...
        stillImageOutput.captureStillImageAsynchronously(from: videoConnection!, completionHandler: { [weak self] (buffer, error) in
            ...
            let newImage = MiniMediaRotate.rotateImage(originImage, withAngle: rotete)
...
extension MiniCameraManager: AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
...
extension MiniCameraManager: CAAnimationDelegate {
